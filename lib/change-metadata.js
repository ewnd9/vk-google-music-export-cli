...
module.exports = function(scriptPath, fetchParams, concurrency) {
    ...
    .then(audios => {
      return Promise
        .map(audios.slice(0, limit), audio => {
          ...
          return spawnShellScript(scriptPath, id, data)
            .then(() => {
...
function spawnShellScript(scriptPath, id, data) {
  return new Promise((resolve, reject) => {
    spawn('sh', [scriptPath, id].concat(data))
